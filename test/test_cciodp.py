...
from xcube_cci.cciodp import find_datetime_format, CciOdp, _run_with_session
...
class CciOdpTest(unittest.TestCase):
    ...
    def test_get_datasets_metadata(self):
        ...
        self.assertEqual([{'long_name': 'Bias of log10-transformed chlorophyll-a concentration in seawater.',
                           'name': 'chlor_a_log10_bias',
                           'units': ''},
                          {'long_name': "Chlorophyll-a concentration in seawater (not log-transformed), "
                                        "generated by SeaDAS using a blended combination of OCI (OC4v6 "
    ...
    def test_get_variables_for_fid(self):
        ...
        self.assertEqual(12, len(variables_dict['esacci.OC.day.L3S.CHLOR_A.multi-sensor.multi-platform.MERGED.3-1.geographic']))
        variable_names = [variable['name'] for variable in variables_dict['esacci.OC.day.L3S.CHLOR_A.multi-sensor.multi-platform.MERGED.3-1.geographic']]
        self.assertEqual(
            ['chlor_a_log10_bias', 'chlor_a', 'MERIS_nobs', 'MODISA_nobs', 'SeaWiFS_nobs', 'VIIRS_nobs', 'total_nobs',
             'chlor_a_log10_rmsd', 'lat', 'lon', 'time', 'crs'], variable_names)
        self.assertTrue('esacci.OC.5-days.L3S.CHLOR_A.multi-sensor.multi-platform.MERGED.3-1.geographic' in variables_dict)
        variable_names = [variable['name'] for variable in variables_dict['esacci.OC.5-days.L3S.CHLOR_A.multi-sensor.multi-platform.MERGED.3-1.geographic']]
        self.assertEqual(
